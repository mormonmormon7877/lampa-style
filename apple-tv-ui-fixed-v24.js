/* Apple-inspired styling for Lampa. Version 24: correct IPTV panel focus when next item is absent. Standalone ES5 plugin. */
(function () {
    'use strict';
...
/* Pause screensaver v23. Independent lifecycle; never changes the movie URL/time. */
(function () {
    'use strict';
    ...
    var paused = false, dead = false, preview = false;
    ...
    function arm() {
        ...
        if (dead || overlay || !paused || get('delay') === '0' || !moviePaused()) return;
        delayTimer = setTimeout(function () {
            if (!paused || !moviePaused() || dead) return;
    ...
    function nextMedia() {
        ...
        if (statusLabel) statusLabel.textContent = 'Заставка v23 • ' + ((fileIndex - 1) % files.length + 1) + '/' + files.length + ' • Загрузка ' + entry.url.split('/').pop() + lastError;
        ...
        if (entry.type === 'video') {
            ...
            item.onplaying = function () {
                ...
                if (statusLabel) statusLabel.textContent = 'Заставка v23 • ' + ((fileIndex - 1) % files.length + 1) + '/' + files.length + ' • ' + entry.url.split('/').pop() + lastError;
                revealMedia();
            };
    ...
    function show(isPreview) {
        ...
        if (isPreview) {
            ...
            statusLabel.textContent = 'Заставка v23';overlay.appendChild(statusLabel);
        }
        document.body.appendChild(overlay);
        tick();clockTimer = setInterval(tick,1000);
        // Keep Lampa's active controller and focused player button intact.
...
/* Correct the initial IPTV panel focus when no next item exists. */
(function () {
    'use strict';
    if (window.__applePanelFocusFix) window.__applePanelFocusFix.destroy();
    var dead = false, bootTimer = null, focusTimer = null, bus = null, attempts = 0;
    function visible(item) { return !!(item && !item.classList.contains('hide') && item.getClientRects().length); }
    function repair() {
        focusTimer = null;
        if (dead || !window.Lampa || !Lampa.Controller || !Lampa.Player) return;
        var state = Lampa.Controller.enabled();
        if (!state || state.name !== 'player_panel') return;
        if (document.getElementById('apple-pause-overlay')) return;
        var data = Lampa.Player.playdata && Lampa.Player.playdata();
        if (!data || !data.iptv) return;
        var panel = document.querySelector('.player .player-panel');
        if (!visible(panel) || visible(panel.querySelector('.player-panel__next'))) return;
        var focused = panel.querySelector('.focus');
        if (!focused || !focused.classList.contains('player-panel__timeline')) return;
        var target = panel.querySelector('.player-panel__playpause');
        if (!visible(target) || !Lampa.Controller.collectionFocus) return;
        Lampa.Controller.collectionFocus(target, panel);
    }
    function changed(event) {
        clearTimeout(focusTimer);
        if (!dead && event && event.name === 'player_panel') focusTimer = setTimeout(repair,0);
    }
    function setup() {
        if (dead) return;
        bus = window.Lampa && Lampa.Controller && Lampa.Controller.listener;
        if (!bus || !bus.follow) {
            if (++attempts < 240) bootTimer = setTimeout(setup,250);
            return;
        }
        bus.follow('toggle',changed);
        var state = Lampa.Controller.enabled();
        if (state) changed(state);
    }
    window.__applePanelFocusFix = {destroy:function () {
        dead = true;clearTimeout(bootTimer);clearTimeout(focusTimer);
        if (bus && bus.remove) bus.remove('toggle',changed);
    }};
    setup();
})();
